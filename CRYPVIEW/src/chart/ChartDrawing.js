...
//  v2.9.1 : Préservation des propriétés de l'ancre lors du drag
//    (bug texte annoté perdu après déplacement)
//  v2.9.2 : Support tactile complet (touchstart / touchmove / touchend)
...
export class ChartDrawing {
  ...
  #bindEvents() {
    ...
    const onMove = (e) => {
      ...
      this.#mousePixel = { x: clientX - r.left, y: clientY - r.top };
    ...
    const onTap = (e) => {
      ...
      const { clientX, clientY } = clientXY(e);
      const r      = this.#canvas.getBoundingClientRect();
      const anchor = this.#pixelToAnchor({ x: clientX - r.left, y: clientY - r.top });
      if (!anchor) return;
    ...
    this.#canvas.addEventListener('touchend', e => { e.preventDefault(); onTap(e); }, { passive: false });
  ...
  #bindEditEvents() {
    ...
    this.#canvas.addEventListener('touchmove', (e) => {
      ...
      const r         = this.#canvas.getBoundingClientRect();
      const t         = e.touches[0];
      const newCoords = this.#pixelToAnchor({ x: t.clientX - r.left, y: t.clientY - r.top });
      if (newCoords) {
        const drawing = this.#drawings.find(d => d.id === this.#dragging.drawingId);
        if (drawing) {
          const existing = drawing.anchors[this.#dragging.anchorIdx];
  ...
  if (type === 'pitchfork' && pts.length >= 3) {
    ...
    const l3End = extend(branches[2], dist);
    g.appendChild(this.#svgEl('polygon', {
      points: `${p2.x},${p2.y} ${l2End.x},${l2End.y} ${cEnd.x},${cEnd.y} ${l3End.x},${l3End.y} ${p3.x},${p3.y} ${p1.x},${p1.y}`,
