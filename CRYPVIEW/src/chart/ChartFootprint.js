...
//    Avant : #data (Map<candleTimeSec, Map>) croissait sans limite.
//    Une session longue sur BTCUSDT/1s pouvait accumuler des
//    milliers d'entrées (~1 Mo/min), soit des centaines de Mo
//    en quelques heures.
//
//    Après : MAX_CANDLES_PER_CACHE = 800 (aligné sur
//    MAX_CANDLES_IN_MEMORY de config.js). #trimDataIfNeeded()
//    évacue le timestamp le plus ancien (ordre d'insertion =
//    ordre chronologique) dès que le seuil est dépassé.
//    Ce correctif s'applique aussi au seed OHLCV (#seed).
//
//  Correctif v2 appliqué :
...
// ── Bornes mémoire ────────────────────────────────────────────
// MAX_CACHE_ENTRIES  : nombre de paires (symbol, tf) gardées en cache LRU.
// MAX_CANDLES_PER_CACHE : nombre max de timestamps par paire.
//   Aligné sur MAX_CANDLES_IN_MEMORY de config.js (800).
//   Garantit que #data ne dépasse jamais 800 × MAX_BUCKETS_PER_CANDLE.
...
export class ChartFootprint {
  ...
  // ── Fix #9 : borne mémoire par clé de cache ───────────────
  /**
   * Évacue le timestamp le plus ancien si #data dépasse
   * MAX_CANDLES_PER_CACHE entrées.
   *
   * Complexité : O(1) — Map.prototype.keys() est un itérateur
   * et .next().value donne le premier élément (insertion order).
   * Comme les insertions sont chronologiques (seed → WS live),
   * le premier inséré est bien le plus vieux timestamp.
   */
  #trimDataIfNeeded() {
  ...
  #seed(candles) {
    for (const c of candles) {
      ...
      // Le seed insère max candles.length entrées — déjà borné par
      // MAX_CANDLES_IN_MEMORY en amont, mais on applique quand même
      // la borne de sécurité pour le cas où le tableau serait plus grand.
      ...
      for (let i = 0; i < trades.length; i += UPGRADE_CHUNK_SIZE) {
        ...
        for (const trade of chunk) {
          ...
          if (!upgradedTimes.has(candleTimeSec)) {
            ...
            // Borne mémoire après chaque nouvelle entrée REST
            this.#trimDataIfNeeded();
          }
  ...
  #insertTrade(price, qty, isBuy, candleTimeSec) {
    ...
    if (!this.#data.has(candleTimeSec)) {
      this.#data.set(candleTimeSec, new Map());
      // Borne mémoire : éviction du plus ancien timestamp si dépassement
      this.#trimDataIfNeeded();
    }
