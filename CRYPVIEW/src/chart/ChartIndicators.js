...
export class ChartIndicators {
  ...
  // ── Cache Ichimoku ────────────────────────────────────────
  // Ichimoku est le calcul le plus lourd (5 boucles × 800 bougies).
  // On mémorise le dernier résultat et on ne recalcule QUE si le
  // timestamp de la dernière bougie a changé (= nouvelle bougie fermée).
  // Structure : { lastCandleTime: number, data: { tenkan, kijun, senkouA, senkouB, chikou } }
  #ichiCache = null;
  ...
  remove(key, hooks = {}) {
    ...
    // ── Invalide le cache si on retire Ichimoku ──────────────
    // Garantit qu'un prochain add('ichi') recalcule depuis zéro.
  ...
  // ── Panneaux sous le chart principal ─────────────────────
  ...
  #applyData(key, ind, candles) {
    ...
    try {
      switch (key) {
        ...
        // ── Ichimoku — calcul mis en cache ──────────────────────────────────────────
        // calcIchimoku est O(n²) : 5 boucles sur jusqu'à 800 bougies avec
        // Math.max(...slice) à chaque itération. On ne recalcule que si le
        // timestamp de la dernière bougie a changé, i.e. une nouvelle bougie
        // vient de se clôturer. Entre deux fermetures (ticks live sur la même
        // bougie), on réutilise le résultat en cache → zéro calcul inutile.
        case 'ichi': {
          const lastTime = candles.at(-1).time;

          // Vérifie si le cache est encore valide
          if (this.#ichiCache?.lastCandleTime !== lastTime) {
            // Nouvelle bougie clôturée → recalcul complet + mise en cache
            const data = calcIchimoku(candles);
            this.#ichiCache = { lastCandleTime: lastTime, data };
          }

          // Applique les données (depuis le cache ou après recalcul)
