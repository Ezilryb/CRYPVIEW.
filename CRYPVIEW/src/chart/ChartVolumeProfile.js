...
//
//  Crée un canvas <canvas id="vp-canvas"> superposé au chart,
//  s'abonne aux redessins de l'axe du temps/prix, et se nettoie
//  proprement via destroy().
// ============================================================
...
export class ChartVolumeProfile {
   ...
   */
  constructor(chart, cSeries, container) {
    this.#chart     = chart;
  ...
  /**
   * Active le Volume Profile pour un jeu de bougies.
  ...
  /**
   * Redessine le profil avec de nouvelles bougies (après un refresh).
   * @param {Candle[]} candles
   */
  redraw(candles) {
    if (!this.#active) return;
  ...
  /** Désactive et nettoie le canvas. */
  deactivate() {
    if (!this.#active) return;
    this.#active = false;
  ...
  /**
   * Calcule le profil de volume par niveaux de prix.
   * Retourne un tableau annoté avec POC, VAH, VAL et Value Area.
   * @param {Candle[]} candles
   * @param {number}   [nBuckets=80]
   * @returns {ProfileBucket[]}
   */
  static calc(candles, nBuckets = 80) {
    ...
    // Distribution du volume par overlap
  ...
  /**
   * Dessine une ligne horizontale avec son étiquette.
   ...
   * @param {number[]} dash
   * @param {string}  label
   * @param {boolean} [labelBelow=false]
   */
  ...
  #ensureCanvas() {
    let canvas = document.getElementById('vp-canvas');
    if (!canvas) {
