...
//      symbols:        allSymbolsArray,
//      recentSymbols:  recentSymbolsInstance,
//      getActiveKeys:  () => indicators.getActiveKeys(),
//      getCurrentSym:  () => chart.symbol,
//      getCurrentTf:   () => chart.timeframe,
//      onSymbol:       (sym)  => connect(sym, tf),
//      onTf:           (tf)   => connect(sym, tf),
//      onToggleInd:    (key)  => toggleIndicator(key),
...
export class CommandPalette {
  ...
  #buildItems() {
    ...
    // ── Indicateurs ───────────────────────────────────────
    const CAT_MAP = {
      trend: 'Indicateurs — Tendance', momentum: 'Indicateurs — Momentum',
      volatility: 'Indicateurs — Volatilité', volume: 'Indicateurs — Volume',
    };
    for (const [key, meta] of Object.entries(IND_META)) {
      items.push({
  ...
  #filter(query) {
    ...
    if (!q) {
      // Sans requête : récents + TF actuel + indicateurs actifs + actions
      ...
      // Injecte une recherche de symboles live si pas encore de résultats sym
  ...
  #bindLocalEvents() {
    ...
    this.#input.addEventListener('input', e => {
      this.#buildItems(); // actualise l'état actif en temps réel
