...
//      onAddAlert:      (clientY) => alertManager.add(...),  // ← v2.7
//    });
...
export class ContextMenu {
   ...
   * @param {function(number)} [callbacks.onAddAlert]  — reçoit clientY du clic droit
  ...
  #bindEvents(chartContainer) {
    ...
    chartContainer.addEventListener('contextmenu', e => {
      e.preventDefault();
      // Stocke la coordonnée Y pour la conversion price-by-pixel dans onAddAlert
    ...
    // ── Alerte de prix ────────────────────────────────────────
    // Transmet la coordonnée Y cliente pour que l'appelant puisse
    // convertir en prix via cSeries.coordinateToPrice().
