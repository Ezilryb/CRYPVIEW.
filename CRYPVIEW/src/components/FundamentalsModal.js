...
class FundamentalsModal {
  ...
  _renderData() {
    ...
    document.getElementById(this._uid('content')).innerHTML = `
      <div class="fm-hero">
        ${d.image?.small ? `<img src="${d.image.small}" alt="${d.name}" style="width:48px;border-radius:50%">` : ''}
        <div>
          <h3 style="margin:0">${d.name} <span style="color:#888;font-weight:400">(${d.symbol?.toUpperCase()})</span></h3>
          <div style="font-size:.85rem;color:#666">Rang #${d.market_cap_rank ?? '--'} — ${d.categories?.slice(0,3).join(', ') || ''}</div>
        </div>
      ...
      <div class="fm-desc">${d.description.en.slice(0,600).replace(/<[^>]+>/g,'')}${d.description.en.length > 600 ? '…' : ''}</div>` : ''}

      <div class="fm-links">
        ${d.links?.homepage?.[0] ? `<a href="${d.links.homepage[0]}" target="_blank" class="fm-link">🌐 Site officiel</a>` : ''}
        ${d.links?.blockchain_site?.[0] ? `<a href="${d.links.blockchain_site[0]}" target="_blank" class="fm-link">🔗 Explorer</a>` : ''}
        ${d.links?.repos_url?.github?.[0] ? `<a href="${d.links.repos_url.github[0]}" target="_blank" class="fm-link">💻 GitHub</a>` : ''}
        ${d.links?.whitepaper ? `<a href="${d.links.whitepaper}" target="_blank" class="fm-link">📄 Whitepaper</a>` : ''}
      </div>`;
  }
