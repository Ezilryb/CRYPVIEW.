...
export class Header {
  ...
  setStatus(state, text) {
    ...
    if (state === 'live') {
      ...
      if (this.#statusText) this.#statusText.textContent = text ?? 'En direct';
    } else if (state === 'reconnecting') {
      this.#dot.classList.add('reconnecting');
      if (this.#statusText) this.#statusText.textContent = text ?? 'Reconnexion…';
    } else {
      // 'offline' | 'connecting' | tout autre état
      if (this.#statusText) this.#statusText.textContent = text ?? 'Connexion…';
    }
  }
