...
// Proxy CORS gratuit et sans clé
const CORS_PROXY = 'https://api.allorigins.win/get?url=';

// Mots-clés pour le filtrage côté client
...
export default class NewsFeedModal {
  constructor() {
    // _id initialisé immédiatement — évite tout problème de this._uid is not a function
    this._id       = Math.random().toString(36).slice(2, 7);
    this._overlay  = null;
    this._articles = [];
    this._filter   = 'all';
    this._loading  = false;
    this._lastFetch = 0; // timestamp du dernier fetch
  }
  ...
  open(/* symbol ignoré — news globales */) {
    ...
    // Re-fetche si pas d'articles ou données > 10 minutes
  ...
  updateSymbol(/* symbol */) {
    // News globales, pas filtrées par paire — rien à faire
  }
  ...
  _buildOverlay() {
    ...
    // IMPORTANT : appendChild AVANT les getElementById
    document.body.appendChild(ov);
    this._overlay = ov;

    // Binding — éléments maintenant dans le DOM
  ...
  /** Fetch + parse un flux RSS via le proxy CORS allorigins */
  ...
  /** Parse le XML RSS et retourne un tableau d'articles normalisés */
  _parseRSS(xmlStr, sourceName) {
    const parser = new DOMParser();
    const doc    = parser.parseFromString(xmlStr, 'text/xml');

    // Gestion erreur de parsing
    ...
    return items.slice(0, 25).map(item => {
      ...
      // Tentatives de récupération de vignette
      const thumbnail =
  ...
  _renderList() {
    ...
    list.innerHTML = filtered.slice(0, 40).map(a => {
      ...
      const img  = a.thumbnail
        ? `<img class="nf-thumb" src="${a.thumbnail}" loading="lazy"
               onerror="this.style.display='none'" alt="">`
        : '';
      const desc = a.desc
        ? `<div class="nf-desc">${a.desc}</div>`
        : '';

      return `
        <a class="nf-item" href="${a.link}" target="_blank" rel="noopener noreferrer">
          ${img}
          <div class="nf-body">
            <div class="nf-meta">
              <span class="nf-source-name">${a.source}</span>
