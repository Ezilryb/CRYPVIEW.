// ============================================================
//  src/indicators/oscillators.js — CrypView V2
//  Oscillateurs et indicateurs de momentum.
//  Fonctions pures — aucun effet de bord.
//
//  Règle cursorrules : "Ne jamais laisser de variables non définies
//  dans les calculs d'indicateurs (ex: MACD)."
//  → Chaque accès optionnel est protégé par ?. ou || 0
//
//  Optimisations O(n) :
//    - calcStoch     : slidingMax/slidingMin remplace Math.max(...slice)
//    - calcWilliamsR : idem
//    - calcCCI       : TP pré-calculé + somme glissante pour la mean
// ============================================================

import { closes, ema, slidingMax, slidingMin } from './moving-averages.js';

/**
 * RSI — Relative Strength Index (Wilder smoothing).
 * @param {Candle[]} candles
 * @param {number}   [period=14]
 * @returns {{ time: number, value: number }[]}
 */
export function calcRSI(candles, period = 14) {
  const cl  = closes(candles);
  const out = [];
  let avgGain = 0;
  let avgLoss = 0;

  // Initialisation : moyenne des gains/pertes sur la première période
  for (let i = 1; i <= period; i++) {
    const diff = cl[i] - cl[i - 1];
    if (diff > 0) avgGain += diff;
    else          avgLoss -= diff;
  }
  avgGain /= period;
  avgLoss /= period;

  for (let i = period; i < cl.length; i++) {
    if (i > period) {
      const diff = cl[i] - cl[i - 1];
      avgGain = (avgGain * (period - 1) + (diff > 0 ? diff : 0)) / period;
      avgLoss = (avgLoss * (period - 1) + (diff < 0 ? -diff : 0)) / period;
    }
    // Protection division par zéro : si avgLoss===0 → RSI=100
    out.push({
      time:  candles[i].time,
      value: avgLoss === 0 ? 100 : 100 - 100 / (1 + avgGain / avgLoss),
    });
  }
  return out;
}

/**
 * MACD — Moving Average Convergence/Divergence.
 * @param {Candle[]} candles
 * @param {number}   [fast=12]
 * @param {number}   [slow=26]
 * @param {number}   [signal=9]
 * @returns {{ macd: Point[], signal: Point[], hist: HistPoint[] }}
 */
export function calcMACD(candles, fast = 12, slow = 26, signal = 9) {
  const cl     = closes(candles);
  const fastEma = ema(cl, fast);
  const slowEma = ema(cl, slow);

  // Mappe les EMA rapides par index pour alignement
  const fastMap = new Map(fastEma.map((x) => [x.i, x.v]));

  // Ligne MACD = EMA_rapide - EMA_lente
  const macdLine = [];
  slowEma.forEach(({ i, v }) => {
    if (fastMap.has(i)) macdLine.push({ i, v: fastMap.get(i) - v });
  });

  // Signal = EMA du MACD
  const signalEma = ema(macdLine.map((x) => x.v), signal);

  const macdOut   = [];
  const signalOut = [];
  const histOut   = [];

  signalEma.forEach(({ i: si, v: sv }) => {
    const m = macdLine[si];
    // Protection : m peut être undefined si l'index dépasse le tableau
    if (!m) return;
    const h = m.v - sv;
    macdOut.push({ time: candles[m.i].time, value: m.v });
    signalOut.push({ time: candles[m.i].time, value: sv });
    histOut.push({
      time:  candles[m.i].time,
      value: h,
      color: h >= 0 ? '#00ff8870' : '#ff3d5a70',
    });
  });

  return { macd: macdOut, signal: signalOut, hist: histOut };
}

/**
 * Stochastique (Fast %K et %D lissé) — O(n) via slidingMax/slidingMin.
 *
 * Avant : Math.max(...slice.map(c => c.high)) dans une boucle → O(n × k).
 * Après : slidingMax/slidingMin pré-calculés une seule fois → O(n).
 *
 * @param {Candle[]} candles
 * @param {number}   [kPeriod=14]
 * @param {number}   [dPeriod=3]
 * @returns {{ k: Point[], d: Point[] }}
 */
export function calcStoch(candles, kPeriod = 14, dPeriod = 3) {
  const highs  = candles.map((c) => c.high);
  const lows   = candles.map((c) => c.low);

  // maxH[j] = max des hauts sur la fenêtre [j .. j + kPeriod - 1]
  // minL[j] = min des bas  sur la fenêtre [j .. j + kPeriod - 1]
  // → bougie de référence = candles[j + kPeriod - 1]
  const maxH = slidingMax(highs, kPeriod);
  const minL = slidingMin(lows,  kPeriod);

  const k = maxH.map((hi, j) => {
    const lo = minL[j];
    const i  = j + kPeriod - 1;
    return {
      time:  candles[i].time,
      value: hi === lo ? 50 : (candles[i].close - lo) / (hi - lo) * 100,
    };
  });

  const d = [];
  for (let i = dPeriod - 1; i < k.length; i++) {
    d.push({
      time:  k[i].time,
      value: (k[i].value + k[i - 1].value + k[i - 2].value) / 3,
    });
  }
  return { k, d };
}

/**
 * CCI — Commodity Channel Index (optimisé O(n) via somme glissante).
 *
 * Avant : slice() + map() à chaque itération → O(n × period) avec
 *         allocations mémoire répétées dans la boucle chaude.
 *
 * Après :
 *   1. Tableau TP pré-calculé une seule fois en Float64Array — O(n).
 *   2. Somme glissante pour la mean : entrée/sortie O(1) par itération.
 *   3. MAD (déviation absolue moyenne) : O(period) incontournable car
 *      |x| est non-linéaire — mais zéro allocation dans la boucle.
 *
 * Gain principal : suppression des slice()/map() dans la boucle chaude
 * et meilleure localité cache grâce au Float64Array contigu.
 *
 * @param {Candle[]} candles
 * @param {number}   [period=20]
 * @returns {{ time: number, value: number }[]}
 */
export function calcCCI(candles, period = 20) {
  const n   = candles.length;
  const out = [];

  // ── Étape 1 : Typical Prices pré-calculés — O(n) ──────────
  // Float64Array : pas de boxing JS, accès cache-friendly.
  const tp = new Float64Array(n);
  for (let i = 0; i < n; i++) {
    tp[i] = (candles[i].high + candles[i].low + candles[i].close) / 3;
  }

  // ── Étape 2 : Amorçage de la somme glissante ──────────────
  // On accumule les (period - 1) premiers TP avant d'entrer
  // dans la boucle principale pour éviter un test conditionnel
  // à chaque itération.
  let windowSum = 0;
  for (let i = 0; i < period - 1; i++) {
    windowSum += tp[i];
  }

  // ── Étape 3 : Boucle principale — O(n × period) ───────────
  // La mean est O(1) grâce à windowSum.
  // La MAD reste O(period) (valeur absolue non-linéaire).
  for (let i = period - 1; i < n; i++) {
    // Entrée du TP courant dans la fenêtre — O(1)
    windowSum += tp[i];
    const mean = windowSum / period;

    // Déviation moyenne absolue — O(period), incontournable
    let md = 0;
    for (let j = i - period + 1; j <= i; j++) {
      md += Math.abs(tp[j] - mean);
    }
    md /= period;

    // Protection division par zéro (prix identiques sur toute la fenêtre)
    out.push({
      time:  candles[i].time,
      value: md ? (tp[i] - mean) / (0.015 * md) : 0,
    });

    // Sortie de l'ancien TP de la fenêtre — O(1)
    windowSum -= tp[i - period + 1];
  }

  return out;
}

/**
 * ADX + DI+/DI- (Average Directional Index avec smoothing de Wilder).
 * @param {Candle[]} candles
 * @param {number}   [period=14]
 * @returns {{ diP: Point[], diN: Point[], adx: Point[] }}
 */
export function calcADX(candles, period = 14) {
  const dmP = [], dmN = [], tr = [];

  for (let i = 1; i < candles.length; i++) {
    const upMove   = candles[i].high  - candles[i - 1].high;
    const downMove = candles[i - 1].low - candles[i].low;
    dmP.push(upMove > downMove && upMove > 0 ? upMove : 0);
    dmN.push(downMove > upMove && downMove > 0 ? downMove : 0);
    tr.push(Math.max(
      candles[i].high - candles[i].low,
      Math.abs(candles[i].high - candles[i - 1].close),
      Math.abs(candles[i].low  - candles[i - 1].close),
    ));
  }

  // Smoothed TR / DM+ / DM- (Wilder)
  const smTR = [], smDMP = [], smDMN = [];
  let t  = tr.slice(0, period).reduce((a, b) => a + b, 0);
  let dp = dmP.slice(0, period).reduce((a, b) => a + b, 0);
  let dn = dmN.slice(0, period).reduce((a, b) => a + b, 0);
  smTR.push(t); smDMP.push(dp); smDMN.push(dn);

  for (let i = period; i < tr.length; i++) {
    t  = t  - t  / period + tr[i];
    dp = dp - dp / period + dmP[i];
    dn = dn - dn / period + dmN[i];
    smTR.push(t); smDMP.push(dp); smDMN.push(dn);
  }

  const diP = [], diN = [];
  smTR.forEach((t, i) => {
    const p = t ? 100 * smDMP[i] / t : 0;
    const n = t ? 100 * smDMN[i] / t : 0;
    diP.push({ time: candles[i + period].time, value: p });
    diN.push({ time: candles[i + period].time, value: n });
  });

  const adx = [];
  for (let i = period - 1; i < diP.length; i++) {
    const slice = diP.slice(i - period + 1, i + 1);
    const dx = slice.map((x, j) => {
      const s = x.value + diN[i - period + 1 + j].value;
      return s ? 100 * Math.abs(x.value - diN[i - period + 1 + j].value) / s : 0;
    });
    adx.push({ time: diP[i].time, value: dx.reduce((a, b) => a + b, 0) / period });
  }

  return { diP, diN, adx };
}

/**
 * Williams %R — O(n) via slidingMax/slidingMin.
 *
 * Avant : Math.max(...slice.map(c => c.high)) dans une boucle → O(n × period).
 * Après : slidingMax/slidingMin pré-calculés une seule fois → O(n).
 *
 * @param {Candle[]} candles
 * @param {number}   [period=14]
 * @returns {{ time: number, value: number }[]}
 */
export function calcWilliamsR(candles, period = 14) {
  const highs = candles.map((c) => c.high);
  const lows  = candles.map((c) => c.low);

  // maxH[j] = max des hauts sur [j .. j + period - 1]
  // minL[j] = min des bas  sur [j .. j + period - 1]
  // → bougie de référence = candles[j + period - 1]
  const maxH = slidingMax(highs, period);
  const minL = slidingMin(lows,  period);

  return maxH.map((hi, j) => {
    const lo = minL[j];
    const i  = j + period - 1;
    return {
      time:  candles[i].time,
      value: hi === lo ? -50 : (candles[i].close - hi) / (hi - lo) * 100,
    };
  });
}

/**
 * MFI — Money Flow Index.
 * @param {Candle[]} candles
 * @param {number}   [period=14]
 * @returns {{ time: number, value: number }[]}
 */
export function calcMFI(candles, period = 14) {
  const out = [];
  for (let i = period; i < candles.length; i++) {
    let posFlow = 0;
    let negFlow = 0;
    for (let j = i - period + 1; j <= i; j++) {
      const tp     = (candles[j].high + candles[j].low + candles[j].close) / 3;
      const prevTp = (candles[j - 1].high + candles[j - 1].low + candles[j - 1].close) / 3;
      const mf     = tp * candles[j].volume;
      if (tp > prevTp) posFlow += mf;
      else             negFlow += mf;
    }
    // Protection division par zéro
    const ratio = negFlow ? posFlow / negFlow : 100;
    out.push({ time: candles[i].time, value: 100 - 100 / (1 + ratio) });
  }
  return out;
}

/**
 * Momentum brut (close[i] - close[i-period]).
 * @param {Candle[]} candles
 * @param {number}   [period=10]
 * @returns {{ time: number, value: number, color: string }[]}
 */
export function calcMom(candles, period = 10) {
  return candles.slice(period).map((c, i) => ({
    time:  c.time,
    value: c.close - candles[i].close,
    color: c.close - candles[i].close >= 0 ? 'rgba(176,255,92,.7)' : 'rgba(255,61,90,.7)',
  }));
}

// ── Nouvelles fonctions ───────────────────────────────────────

/**
 * OBV — On-Balance Volume.
 * Cumule le volume positivement sur les jours haussiers, négativement sur les baissiers.
 * @param {Candle[]} candles
 * @returns {{ time: number, value: number }[]}
 */
export function calcOBV(candles) {
  const out = [];
  let obv = 0;
  for (let i = 1; i < candles.length; i++) {
    const c = candles[i], p = candles[i - 1];
    if (c.close > p.close)      obv += c.volume;
    else if (c.close < p.close) obv -= c.volume;
    // c.close === p.close → obv inchangé
    out.push({ time: c.time, value: obv });
  }
  return out;
}

/**
 * TRIX — taux de variation de la Triple EMA (oscillateur momentum).
 * Valeur positive = tendance haussière, négative = baissière.
 *
 * Alignement e3 :
 *   e3[k] → candle index k + 3*(period-1)
 *
 * @param {Candle[]} candles
 * @param {number}  [period=18]
 * @returns {{ time: number, value: number, color: string }[]}
 */
export function calcTRIX(candles, period = 18) {
  const cl = closes(candles);
  const e1 = ema(cl, period);
  const e2 = ema(e1.map(x => x.v), period);
  const e3 = ema(e2.map(x => x.v), period);

  return e3.map(({ v }, k) => {
    if (k === 0) return null;
    const prev    = e3[k - 1].v;
    const trix    = prev !== 0 ? ((v - prev) / prev) * 100 : 0;
    const cIdx    = k + 3 * (period - 1);
    return {
      time:  candles[cIdx].time,
      value: trix,
      color: trix >= 0 ? '#00ff8870' : '#ff3d5a70',
    };
  }).filter(Boolean);
}

/**
 * CMF — Chaikin Money Flow.
 * Flux monétaire normalisé sur la période : positif = pression acheteuse.
 * @param {Candle[]} candles
 * @param {number}  [period=20]
 * @returns {{ time: number, value: number }[]}
 */
export function calcCMF(candles, period = 20) {
  const out = [];
  for (let i = period - 1; i < candles.length; i++) {
    let mfv = 0, vol = 0;
    for (let j = i - period + 1; j <= i; j++) {
      const c     = candles[j];
      const range = c.high - c.low;
      const clv   = range > 0 ? ((c.close - c.low) - (c.high - c.close)) / range : 0;
      mfv += clv * c.volume;
      vol += c.volume;
    }
    out.push({ time: candles[i].time, value: vol ? mfv / vol : 0 });
  }
  return out;
}

/**
 * Elder Ray Index : Bull Power (High − EMA) et Bear Power (Low − EMA).
 * Bull > 0 + Bear < 0 → équilibre ; divergences = signaux directionnels.
 * @param {Candle[]} candles
 * @param {number}  [period=13]
 * @returns {{ bull: HistPoint[], bear: HistPoint[] }}
 */
export function calcElderRay(candles, period = 13) {
  const cl     = closes(candles);
  const emaArr = ema(cl, period);
  const bull = [], bear = [];
  emaArr.forEach(({ i, v }) => {
    bull.push({ time: candles[i].time, value: candles[i].high - v, color: '#00ff8870' });
    bear.push({ time: candles[i].time, value: candles[i].low  - v, color: '#ff3d5a70' });
  });
  return { bull, bear };
}