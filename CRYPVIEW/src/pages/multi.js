...
export class MultiChartView {
  ...
  async init() {
    ...
    // ── MobileToolbar — pointe sur le drawing du panneau actif ──
    this.#mobileToolbar = new MobileToolbar({
      drawing:      this.#activeInst?.drawing ?? null,
      onIndicators: () => this.#indModal?.open(),
      onScreener:   () => this.#screenerModal?.open(),
    });
  ...
  #setActive(idx) {
    ...
    // Rebind MobileToolbar sur le drawing du nouveau panneau actif
    this.#mobileToolbar?.setDrawing(inst.drawing);
  }
