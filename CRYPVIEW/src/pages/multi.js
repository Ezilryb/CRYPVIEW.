...
export class MultiChartView {
  ...
  #saveState() {
    ...
    // ── NOUVEAU : resync après tout changement de chart ──────
    this.#rebuildChartSync();
    // ─────────────────────────────────────────────────────────
  }
  ...
  /**
   * Reconstruit le groupe de sync depuis les instances courantes.
   * Appelé après init ET après chaque reconnect (via #saveState).
   */
  #rebuildChartSync() {
    ...
    // Préserve l'état des boutons si déjà créés
  ...
  /**
   * Barre de contrôle sync : injectée entre le header et la grille.
   */
  #buildSyncToolbar() {
    ...
    const makeBtn = (id, label, title, defaultOn = true) => {
  ...
  /**
   * Initialise la palette de commandes en mode multi.
   */
  #initCommandPalette() {
...
class MultiChartInstance {
  ...
  #connectTicker() {
    ...
    this.#wsTicker.onMessage = (msg) => {
      ...
      // Mise à jour pctChange24h pour les alertes
