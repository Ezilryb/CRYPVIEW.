...
//    Le getter pointe toujours vers l'instance active au moment du
//    rendu → aucun cache interne à synchroniser manuellement.
//    Tous les appels render(arg) remplacés par render() sans argument.
// ============================================================
...
export class MultiChartView {
  ...
  #buildSharedComponents() {
    ...
    this.#ctxMenu = new ContextMenu(
      ...
      {
        onOpenIndModal: () => {
          // open() appelle render() via le getter — toujours synchro avec l'instance active
          this.#indModal.open();
        },
        onRemoveAllInd: () => {
          this.#activeInst.removeAllIndicators();
          // render() sans argument — le getter retournera [] après removeAll
    ...
    // ── Fix #8 : getter pointant vers l'instance active au moment du rendu ──
    // Quand l'utilisateur change de panneau actif, le getter retourne
    // automatiquement les clés du bon panneau sans aucune mise à jour manuelle.
    this.#indModal = new IndicatorModal({
      getActiveKeys: () => this.#activeInst?.indicators?.getActiveKeys() ?? [],

      onAdd: key => {
        this.#activeInst.addIndicator(key);
        // render() sans argument — getter toujours frais
        this.#indModal.render();
      },
      onRemove: key => {
      ...
      switch (e.key) {
        ...
        case 'I':
          if (!e.ctrlKey && !e.metaKey && !e.altKey) {
            e.preventDefault();
            // open() appelle render() via le getter pointant sur inst actif
            this.#indModal.open();
          }
          break;
