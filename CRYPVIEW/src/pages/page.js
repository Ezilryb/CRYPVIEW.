// ============================================================
//  src/pages/page.js — CrypView V2.7
...
// ── Helper debounce (perf recherche symbole) ──────────────────
...
/**
 * Synchronise les PriceLines LightweightCharts avec les alertes actives
 * pour le symbole courant. Appelé après chaque modification de la liste
 * d'alertes et après chaque changement de symbole/TF.
 *
 * Principe :
 *  - Supprime les lines dont l'alerte n'existe plus (ou appartient à un autre symbole)
 *  - Crée les lines pour les nouvelles alertes du symbole courant
 *  - Ne touche pas aux lines qui n'ont pas changé (évite le clignotement)
 */
    ...
    if (!alertPriceLines.has(alert.id)) {
      try {
        const line = chart.cSeries.createPriceLine({
          ...
          color:            '#ff9900',
          lineWidth:        1,
          lineStyle:        2,   // LightweightCharts : LineStyle.Dashed = 2
...
/**
 * Supprime toutes les PriceLines sans nettoyer la Map interne.
 * Utilisé avant une réinitialisation du chart (changement de symbole).
 */
...
async function boot() {
  ...
  // Synchronisation initiale : rend les alertes persistées de la session précédente
...
async function connect(symbol, timeframe) {
  ...
  // Supprime les price lines du symbole précédent avant de rechanger
  ...
  // Redessine les price lines pour le nouveau symbole
  syncAlertPriceLines();
}
...
function wireChartEvents(container) {
  ...
  container.addEventListener('crypview:history:loaded', ({ detail }) => {
    ...
    // Après rechargement d'historique, les price lines doivent être recréées
    // (les séries LightweightCharts sont réinitialisées via setData)
    clearAllAlertPriceLines();
    syncAlertPriceLines();
  });
}
...
function initContextMenu(container) {
  ...
  ctxMenu = new ContextMenu(container, {
    ...
    // ── v2.7 : Alerte de prix ─────────────────────────────────
    // clientY est la coordonnée verticale du clic droit dans la fenêtre.
    // On la convertit en prix via la série de bougies de LightweightCharts.
    onAddAlert: async (clientY) => {
      ...
      // Demande de permission (suite à un geste utilisateur = le clic) 
      const perm = await alertManager.requestPermission();
      if (perm === 'denied') return; // bloqué → on n'ajoute pas l'alerte

      const lastClose = chart.candles.at(-1)?.close ?? price;
      alertManager.add(chart.symbol, price, lastClose);
      // syncAlertPriceLines() est appelé par alertManager.onAlertsChange
    },
  });
}
...
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'hidden') {
    ...
    chart?.destroy();
    sidebar?.reset();
    header?.setStatus('offline', 'En pause (onglet en arrière-plan)');
  } else if (autoPaused) {
    autoPaused = false;
    const container = $('chart-container');
    ...
    initChartModules(container);
    alertManager.onAlertsChange = syncAlertPriceLines;
    chart.start().then(() => syncAlertPriceLines());
  }
});
