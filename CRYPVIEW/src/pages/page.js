...
function destroyModules() {
  ...
  footprint?.destroy();
  orderflow?.destroy();
  // ctxMenu, themeToggle, indModal, settingsModal survivent au cycle pause/reprise
  indicators = null;
  vp         = null;
  footprint  = null;
  orderflow  = null;
...
window.addEventListener('beforeunload', () => {
  destroyModules();
  themeToggle?.destroy();   // ← nettoyage final (keydown listeners, etc.)
  settingsModal?.destroy(); // ← idem
  chart?.destroy();
});
