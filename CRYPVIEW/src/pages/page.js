...
//
//  v2.7   : AlertManager (alertes de prix + price lines)
//  v2.7.1 : AlertPriceModal (saisie du prix cible)
//  v2.7.2 : AlertListModal (gestion / suppression des alertes)
//  v2.8   : Raccourcis clavier (I, Ctrl+Z, T, Échap via ChartDrawing)
//  v2.10  : Fix #8 — passage du getter getActiveKeys à IndicatorModal ;
//            render() désormais sans argument dans toute la page.
//
//  Correctifs appliqués :
//    - Bug 2 : ajout des hooks onActivateVP / onDeactivateVP dans makeHooks()
//              → Volume Profile désormais fonctionnel depuis l'IndicatorModal.
//    - Bug 3 : connect() utilise chart.changeTo() — un seul appel REST
//              quelle que soit la combinaison symbole/TF.
//              wasVP correctement capturé et restauré.
// ============================================================
    ...
    switch (e.key) {
      ...
      case 'I':
        if (!e.ctrlKey && !e.metaKey && !e.altKey) {
          e.preventDefault();
          // open() appelle render() qui utilise le getter — toujours frais
          indModal?.open();
        }
        break;
...
function initChartModules(container) {
  ...
  indicators.onStateChange = (_key, _active) => {
    updateIndBar();
    // render() sans argument — le getter garantit l'état courant
...
//
//  Fix #8 : on passe un getter getActiveKeys au lieu d'appeler
//  render(activeKeys) manuellement. La modal est toujours en
//  phase avec la source de vérité sans aucun cache interne.
// ══════════════════════════════════════════════════════════════

function initIndicatorModal() {
  indModal = new IndicatorModal({
    // Getter — source de vérité unique pour l'état des indicateurs
    getActiveKeys: () => indicators?.getActiveKeys() ?? [],

    onAdd: (key) => {
      indicators?.add(key, chart.candles, makeHooks());
      // render() sans argument : le getter fournit l'état frais
      indModal.render();
    },
    onRemove: (key) => {
      indicators?.remove(key, makeHooks());
