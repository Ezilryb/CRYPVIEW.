...
async function boot() {
  ...
  // Synchronise price lines ET rafraîchit la liste si elle est ouverte
  ...
  // Raccourcis clavier globaux (montés en dernier pour que les refs soient prêtes)
  mountKeyboardShortcuts();
}

// ══════════════════════════════════════════════════════════════
//  RACCOURCIS CLAVIER
//    I       → ouvre la modal indicateurs
//    Ctrl+Z  → défait le dernier tracé
//    T       → cycle du timeframe suivant
//    Échap   → annule l'outil de dessin actif (géré par ChartDrawing)
// ══════════════════════════════════════════════════════════════
...
function mountKeyboardShortcuts() {
  document.addEventListener('keydown', (e) => {
    ...
    switch (e.key) {
      ...
      // T → cycle vers le timeframe suivant
      case 't':
      case 'T':
        if (!e.ctrlKey && !e.metaKey && !e.altKey) {
          e.preventDefault();
      ...
      // Échap → géré directement par ChartDrawing (pas de duplication)
    }
  });
}

/**
 * Passe au timeframe suivant dans la liste ALL_TF (cycle infini).
 * Met à jour le bouton actif dans la barre TF, puis reconnecte.
 */
function cycleTf() {
  if (!chart) return;
