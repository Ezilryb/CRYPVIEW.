...
// ✅ ALL_TF est la source de vérité unique pour les timeframes disponibles.
//    En l'important ici, la barre TF de page.html reste synchronisée
//    avec les vues multi-graphiques sans aucun hardcoding en HTML.
...
// Doit correspondre à une valeur de ALL_TF.tf.
const DEFAULT_TF = '1s';
...
async function boot() {
  ...
  // 1. Sidebar — en premier : ne dépend d'aucun autre module
  sidebar = new Sidebar();
...
//
//  ── Génération dynamique depuis ALL_TF ──────────────────────
//  Les boutons sont créés en JS (pas en HTML) via ALL_TF importé
//  depuis TimeframeBar.js.  C'est la seule source de vérité pour
//  la liste des TF dans tout le projet :
//    • page.html  → boutons générés ici
//    • multi2/4   → grille générée par TimeframeBar#buildGrid()
//  → Ajouter/supprimer un TF = modifier uniquement ALL_TF.
// ══════════════════════════════════════════════════════════════
...
function mountTimeframeBar() {
  ...
  // ── Construction des boutons depuis ALL_TF ─────────────────
  ...
  // ── Scroll horizontal : molette + drag ────────────────────
  const STEP = 47;
...
function destroyModules() {
  ...
  footprint?.destroy();
  orderflow?.destroy();
  // sidebar, ctxMenu, themeToggle, indModal, settingsModal survivent au cycle pause/reprise
  indicators = null;
  vp         = null;
  footprint  = null;
  orderflow  = null;
...
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'hidden') {
    ...
    // Remet le bouton TF actif dans la barre
    const strip = $('tf-scroll');
    strip?.querySelectorAll('.tf-btn').forEach(b => {
