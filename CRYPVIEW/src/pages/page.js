...
import { Sidebar }            from '../components/Sidebar.js';     // ✅ ajouté
...
let sidebar       = null;   // Sidebar                            // ✅ ajouté
...
async function boot() {
  ...
  // 1. Sidebar — en premier : ne dépend d'aucun autre module       // ✅ ajouté
  sidebar = new Sidebar();
...
async function connect(symbol, timeframe) {
  ...
  // ✅ Vide la liste des trades avant de changer de paire
  sidebar?.clearTrades();
...
function wireChartEvents(container) {
  ...
  // ✅ Délégué à Sidebar.updateStats() — supprime la manipulation DOM inline
  container.addEventListener('crypview:ticker:update', ({ detail }) => {
    sidebar?.updateStats(detail);
  });

  // ✅ Délégué à Sidebar.addTrade() — supprime la manipulation DOM inline
...
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'hidden') {
    ...
    destroyModules();
    chart?.destroy();
    sidebar?.reset();   // ✅ remet les stats à '—' et vide les trades
