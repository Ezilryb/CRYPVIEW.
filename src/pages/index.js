...
(function initFootprintPreview() {
  ...
  const W_CSS  = canvas.getBoundingClientRect().width  || canvas.offsetWidth  || Math.min(window.innerWidth - 40, 420);
  const H_CSS  = canvas.getBoundingClientRect().height || canvas.offsetHeight || 140;
...
(function initOrderflowPreview() {
  ...
  function drawCVD() {
    if (!cvdCanvas) return;
    const dpr = window.devicePixelRatio || 1;
    const W = wrap.getBoundingClientRect().width || wrap.offsetWidth || Math.min(window.innerWidth - 40, 420);
    const H   = 40;
